...
**Mensagem reescrita por IA sob demanda** (migrations `019`/`022`, corrige o desenho da `018` que era automático em background — não era o que o operador precisava): no modal de Follow-up, o campo **Data** já vem preenchido com **amanhã** (folga de 1 dia até o cron das 9h). O botão **"✨ Reescrever com IA"** troca o rascunho pelo texto gerado, sem sair do modal:

- O role `anon` tem `statement_timeout = 3s` (guarda de segurança do Supabase, vale pra toda query do app) — curto demais pra esperar uma LLM responder (`SET LOCAL` dentro da função **não** ajuda: o prazo do statement já está fixado no valor de quando ele começou, mudar no meio não estende). Por isso o fluxo é em **duas chamadas RPC rápidas**, não uma só bloqueada:
  1. `monjaro.iniciar_reescrita_ia(rascunho, nome_cliente)` — dispara a chamada à API de chat completions via `pg_net` (fire-and-forget) e devolve o `request_id` na hora (milissegundos).
  2. `monjaro.checar_resposta_ia(request_id)` — o client chama a cada ~1s (até 20x) até vir o texto pronto; cada chamada é uma leitura simples de `net._http_response`, sempre rápida.
- O rascunho digitado vira a base do prompt (a IA varia o texto, não descarta).
- Se a IA não responder a tempo, falhar, ou `monjaro.config` não tiver `ai_chat_url`/`ai_chat_token`/`ai_model`, o botão mostra erro (toast) e **o rascunho original permanece no campo** — o operador decide se tenta de novo ou agenda como está.
- Não há automação em background: só roda quando o operador clica no botão.
