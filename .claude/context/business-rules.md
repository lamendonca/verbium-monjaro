...
**Mensagem reescrita por IA sob demanda** (migration `019`, corrige o desenho da `018` que era automático em background — não era o que o operador precisava): no modal de Follow-up, o campo **Data** já vem preenchido com **amanhã** (folga de 1 dia até o cron das 9h). O botão **"✨ Reescrever com IA"** chama a função `monjaro.reescrever_mensagem_ia(rascunho, nome_cliente)` via RPC direto do client — **síncrona**: dispara a chamada à API de chat completions via `pg_net` e aguarda a resposta com um polling curto (`pg_sleep`, até ~8s) dentro do próprio statement, devolvendo o texto pronto pro modal. O rascunho digitado vira a base do prompt (a IA varia o texto, não descarta). Se a IA não responder a tempo, falhar, ou `monjaro.config` não tiver `ai_chat_url`/`ai_chat_token`/`ai_model`, a chamada retorna erro claro (toast) e **o rascunho original permanece no campo** — o operador decide se tenta de novo ou agenda como está. Não há automação em background: só roda quando o operador clica.
